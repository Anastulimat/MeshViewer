...
EVT_CHAR_HOOK(myFrameGUI::onKeyboardEvent)
...
/*****FILE MENU********************************************************/
	{
		...
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_OPEN);
		fileMenu->Append(wxID_REVERT_TO_SAVED, wxT("&Reload Mesh\tCtrl+R"));
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_REVERT_TO_SAVED);
		fileMenu->Append(wxID_SAVEAS, wxT("&Save Mesh to File\tCtrl+S"));
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_SAVEAS);
		fileMenu->AppendSeparator();
		fileMenu->Append(wxID_EXIT, wxT("E&xit\tCtrl+X"));
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_EXIT);
		...
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_HELP);
		helpMenu->Append(wxID_VIEW_DETAILS, wxT("&How to use this interface"));
		Bind(wxEVT_COMMAND_MENU_SELECTED, &myFrameGUI::onMenuEvent, this, wxID_VIEW_DETAILS);
...
/***TOPPANEL CONTROLS*********************************************************************/
	{
		...
		for (auto it = checkboxes.begin(); it != checkboxes.end(); ++it)
		{
			...
			Bind(wxEVT_CHECKBOX, &myFrameGUI::onButtonsEvent, this, _id);
		}
		...
		for (auto it = spinctrls.begin(); it != spinctrls.end(); ++it)
		{
			...
			Bind(wxEVT_SPINCTRL, &myFrameGUI::onButtonsEvent, this, _id);
		}
		...
		for (auto it = buttons.begin(); it != buttons.end(); ++it)
		{
			...
			Bind(wxEVT_BUTTON, &myFrameGUI::onButtonsEvent, this, _id);
		}
		...
		Bind(wxEVT_RADIOBOX, &myFrameGUI::onButtonsEvent, this, ID);
		...
		Bind(wxEVT_LISTBOX, &myFrameGUI::onButtonsEvent, this, ID);
		...
		wrapping_sizer->Add(get<0>(spinctrls[spinctrl_bumpen]), 0, wxGROW | wxALL, 5);
		wrapping_sizer->Add(get<0>(buttons[button_bumpen]), wxSizerFlags().Expand().Bottom());
		...
		wrapping_sizer = new wxStaticBoxSizer(new wxStaticBox(get<0>(tabpanels[SUBDIVISION]), wxID_ANY, wxT(""), wxDefaultPosition, wxDefaultSize), wxVERTICAL);
		sizer_tabpanels->Add(wrapping_sizer, 0, wxGROW | wxALL, 5);
		wrapping_sizer->Add(get<0>(buttons[button_splitface_two]), wxGROW | wxALL);
		...
		wrapping_sizer->Add(get<0>(checkboxes[checkbox_showbvhintersection]), wxSizerFlags().Expand());
...
void myFrameGUI::onMenuEvent(wxCommandEvent & event)
{
	...
	switch (event.GetId())
	{
		...
		case wxID_HELP:
		{
			wxMessageBox(wxT("MeshTeacher: copyright Nabil Mustafa."));
...
void myFrameGUI::onKeyboardEvent(wxKeyEvent& event)
{
...
void myFrameGUI::onButtonsEvent(wxCommandEvent &event)
{
	...
	case checkbox_showbvhintersection:
	{
		break;
	}
	...
	case button_splitface_two:
	{
		if (canvas->faces_todraw[myGLCanvasGUI::SELECTED].size() && canvas->points_todraw[myGLCanvasGUI::SELECTED].size())
		{
			myFace *f = canvas->faces_todraw[myGLCanvasGUI::SELECTED].back();
			glm::vec3 p = canvas->points_todraw[myGLCanvasGUI::SELECTED].back();
			cout << "Code here to split face into QUADS.\n";
			canvas->mesh->splitFaceQUADS(f, p);
			canvas->mesh->computeNormals();
			reset();
		}
		else cout << "Unable to split face.\n";
	...
	case button_bumpen:
	{
		if (canvas->vertices_todraw[myGLCanvasGUI::SELECTED].size())
		{
			myVertex *v = canvas->vertices_todraw[myGLCanvasGUI::SELECTED].back();
			canvas->mesh->bumpen(v, 0.01f, get<0>(spinctrls[spinctrl_bumpen])->GetValue());
			canvas->mesh->computeNormals();
			canvas->reset();
		}
		else cout << "Unable to bumpen vertex.\n";
		break;
	}
...
void myFrameGUI::updateDataStructureInfo(myHalfedge *e)
{
	...
	std::stringstream ss;
	ss << std::hex << std::noshowbase;
	ss << "-----HALFEDGE:     " << reinterpret_cast<intptr_t>(e) << "---------\r\n";
