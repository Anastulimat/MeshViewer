...
void myGLCanvasGUI::closestObject(mySegment &ray, glm::vec3 & p, myVertex * & v, myHalfedge * & e, myFace * & f) const
{
...
void myGLCanvasGUI::computeDataStructureInfo(int mouse_x, int mouse_y)
{
	...
	mySegment ray = camera->constructRay(mouse_x, mouse_y);
	closestObject(ray, p, v, e, f);
...
void myGLCanvasGUI::OnMouse(wxMouseEvent& event)
{
	...
	if (event.LeftDown())
	{
		if (wxGetKeyState(WXK_CONTROL))
		{
			...
			mySegment ray = camera->constructRay(event.GetX(), event.GetY());
			...
			if (f)
			{
				...
				if (get<0>(main_window->checkboxes[myFrameGUI::checkbox_selectfaces])->GetValue() && std::find(faces_todraw[SELECTED].begin(), faces_todraw[SELECTED].end(), f) == faces_todraw[SELECTED].end() ) 
					faces_todraw[SELECTED].push_back(f);
				if (get<0>(main_window->checkboxes[myFrameGUI::checkbox_selectpoints])->GetValue()) 
					points_todraw[SELECTED].push_back(p);
				if (get<0>(main_window->checkboxes[myFrameGUI::checkbox_selectedges])->GetValue() && e && std::find(edges_todraw[SELECTED].begin(), edges_todraw[SELECTED].end(), e) == edges_todraw[SELECTED].end() ) 
					edges_todraw[SELECTED].push_back(e);
				if (get<0>(main_window->checkboxes[myFrameGUI::checkbox_selectvertices])->GetValue() && v && std::find(vertices_todraw[SELECTED].begin(), vertices_todraw[SELECTED].end(), v) == vertices_todraw[SELECTED].end() )
					vertices_todraw[SELECTED].push_back(v);

				main_window->updateMeshInfo();
			}
			else cout << "No object selected.\n";

			if (bvh != nullptr && get<0>(main_window->checkboxes[myFrameGUI::checkbox_showbvhintersection])->GetValue())
			{
				float t;
				bvh->bvh_root->box.intersect(ray, t);
				glm::vec3 box_point = ray.p1 + t * (ray.p2 - ray.p1);
				points_todraw[SELECTED].push_back(box_point);
			}
